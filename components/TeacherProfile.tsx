  ...
  return (
    <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-soft h-full flex flex-col transition-colors duration-200">
      <div className="text-center mb-6">
        <div className="w-24 h-24 mx-auto bg-gradient-to-br from-primary to-secondary rounded-full flex items-center justify-center text-white text-3xl font-bold mb-3 shadow-glow overflow-hidden">
          <img src="/avatar.jpg" alt="Hoàng Thị Hoàn" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
          <span className="absolute -z-10">HH</span>
        </div>
      ...
      <div className="space-y-4 mb-8">
        <div className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-300">
          ...
          <span>Giáo viên Tin học</span>
        </div>
        <div className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-300">
          <BookOpen size={18} className="mt-0.5 text-primary min-w-[18px]" />
          <span>Phong cách: Vui vẻ, hài hước, mang lại năng lượng tích cực.</span>
        </div>
        <div className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-300">
          <Clock size={18} className="mt-0.5 text-primary min-w-[18px]" />
          <span>Địa chỉ: xã Liên Hiệp, tỉnh Tuyên Quang.</span>
        </div>
      </div>
